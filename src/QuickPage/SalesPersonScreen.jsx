...
const SalesPersonScreen = () => {
  ...
  const [newCommissionRate, setNewCommissionRate] = useState(salesPersonData.salesCommissionRate);
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <h2 className="text-2xl font-semibold text-center">Sales Person Screen</h2>

      {/* Sales Person Information */}
      <div className="bg-white p-6 shadow-lg rounded-lg">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <p className="font-medium text-gray-700">Sales Person Name: {salesPersonData.name}</p>
            <p className="text-gray-600">Account: {salesPersonData.account}</p>
            <p className="text-gray-600">Contract: {salesPersonData.contract}</p>
          </div>
          <div>
            <p className="text-gray-600">
              Sales Commission Rate: {isEditing ? (
                <input
                  type="number"
