...
const QuoteEstimateScreen = () => {
  ...
  const handleChangeSalesPerson = () => {
    if (isUserAllowed) {
      setQuoteData(prevData => ({ ...prevData, salesPerson: newSalesPerson }));
      setNewSalesPerson('');
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <h2 className="text-2xl font-semibold text-center">Quote / Estimate Screen</h2>

      {/* Quote Information */}
      <div className="bg-white p-4 shadow rounded-lg">
        <div className="flex justify-between">
          <p className="font-medium text-gray-700">Quote Number: {quoteData.quoteNumber}</p>
          <p className="font-medium text-gray-700">Account: {quoteData.accountName}</p>
        </div>
        ...
        <div className="flex justify-between mt-2">
          <p className="text-gray-600">Reference Payment: {quoteData.referencePayment}</p>
          <p className="font-bold text-gray-700">Total: ${quoteData.totalAmount}</p>
        </div>
      </div>
      ...
      {isUserAllowed && (
        <div className="mt-6 bg-white p-6 shadow-lg rounded-lg max-w-md mx-auto">
          ...
          <p className="text-sm text-gray-600">Are you sure you want to void the quote?</p>
          <button
