...
const ReceiveItems = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <h2 className="text-2xl font-semibold mb-6 text-center">Receive Items</h2>
      ...
      <div className="mt-8">
        <h3 className="text-xl font-semibold text-center mb-4">Received Items</h3>
        <ul className="space-y-2">
          {receivedItems.map((item) => (
            <li key={item.id} className="p-4 border border-gray-200 rounded-lg shadow-sm">
              ...
              {item.serialNumber && <p className="text-sm text-gray-600">Serial: {item.serialNumber}</p>}
              <p className="text-sm text-gray-600">Site: {item.site}</p>
              {item.poNumber && <p className="text-sm text-gray-600">PO Number: {item.poNumber}</p>}
            </li>
          ))}
        </ul>
      </div>
