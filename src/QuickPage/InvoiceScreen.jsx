...
const InvoiceScreen = () => {
  ...
  const handleChangeSalesPerson = () => {
    if (isUserAllowed) {
      setInvoiceData(prevData => ({ ...prevData, salesPerson: newSalesPerson }));
      setNewSalesPerson('');
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <div className="bg-white p-6 shadow-lg rounded-lg">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <p className="font-medium text-gray-700">Invoice Number: {invoiceData.invoiceNumber}</p>
            <p className="text-gray-600">Account: {invoiceData.accountName}</p>
            <p className="text-gray-600">Contract: {invoiceData.contract}</p>
            <p className="text-gray-600">Reference Payment: {invoiceData.referencePayment}</p>
          </div>
          <div>
            <p className="text-gray-600">Sales Person: {invoiceData.salesPerson}</p>
            <p className="text-gray-600">Sales Commission: ${invoiceData.salesCommission}</p>
            <p className="font-bold text-gray-700">Total: ${invoiceData.totalAmount}</p>
          </div>
        </div>
      </div>
      ...
      {isUserAllowed && (
        <div className="mt-6 bg-white p-6 shadow-lg rounded-lg max-w-md mx-auto">
          <h3 className="text-xl font-semibold text-center">Change Sales Person</h3>
          <input
            type="text"
      ...
      {isUserAllowed && (
        <div className="mt-6 bg-white p-6 shadow-lg rounded-lg max-w-md mx-auto">
          ...
          <p className="text-sm text-gray-600">Are you sure you want to void this invoice?</p>
          <button
